...
_DLL_DIR = os.path.join(_HERE, "lib", "bin")

def _ensure_dll_dir_on_windows():
    if sys.platform.startswith("win"):
        # Python 3.8+ requires explicit DLL dir
        add_dir = getattr(os, "add_dll_directory", None)
        if callable(add_dir):
            add_dir(_DLL_DIR)
        else:
            # Fallback for very old Python: prepend to PATH
            os.environ["PATH"] = _DLL_DIR + os.pathsep + os.environ.get("PATH", "")

_ensure_dll_dir_on_windows()

arch = platform.architecture()
if arch[1] == "WindowsPE":
    if arch[0] == "64bit":
        dll_path = os.path.join(_DLL_DIR, "libjbigdec-w64.dll")
    else:
        dll_path = os.path.join(_DLL_DIR, "libjbigdec-w32.dll")
    libjbigdec = cdll.LoadLibrary(dll_path)
else:
    # *nix: keep as-is but still relative to this file
    dll_path = os.path.join(_HERE, "libjbigdec.so")
    libjbigdec = cdll.LoadLibrary(dll_path)
