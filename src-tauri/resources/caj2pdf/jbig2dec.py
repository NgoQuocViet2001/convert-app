...
if arch[1] == "WindowsPE":
    if arch[0] == "64bit":
        dll_path = os.path.join(_DLL_DIR, "libjbig2codec-w64.dll")
    else:
        dll_path = os.path.join(_DLL_DIR, "libjbig2codec-w32.dll")
    libjbig2codec = cdll.LoadLibrary(dll_path)
else:
    dll_path = os.path.join(_HERE, "libjbig2codec.so")
    libjbig2codec = cdll.LoadLibrary(dll_path)
